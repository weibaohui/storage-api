...
func (r *instance) ListSnapshot() ([]*api.Snapshot, error) {
	config := fmt.Sprintf(`
	{"limit":20,"start":0,"sort":""}
	`)
	return r.listSnapshot(config)
}

//列表显示快照，按名称查询，名称唯一
func (r *instance) ListSnapshotByName(name string) ([]*api.Snapshot, error) {
	config := fmt.Sprintf(`
	{"limit":20,"start":0,"sort":"",
...
func (r *instance) listSnapshot(config string) ([]*api.Snapshot, error) {
	url := r.common.Command("/commands/get_snapshot.action")
...
func (r *instance) CreateSnapshot(name, path, desc string, expireTime int) (id string, err error) {
	url := r.common.Command("/commands/create_snapshot.action")
	params := make(map[string]string, 0)
	...
	`, name, r.common.StoreName, path, desc, expireTime, r.common.Username)
	str, err := r.common.PostWithLoginSession(url, params)
	if err != nil {
		return "", err
	}
	...
	//找到快照ID
	snapshots, err := r.ListSnapshotByName(name)
	if err != nil {
		return "", err
	}
...
func (r *instance) RevertSnapshot(id string) (ok bool, err error) {
	url := r.common.Command("/commands/revert_snapshot.action")
	params := make(map[string]string, 0)
	...
	//等待job执行完成
	ok, err = r.isJobDone(result.Data.JobIDStr)
	if err != nil {
		return false, err
	}
...
//params: {"ids":[3]}
//{"detail_err_msg":"","err_msg":"","err_no":0,"result":{"job_id":1603862185597684,"job_id_str":"1603862185597684"},"sync":false,"time_stamp":1566271665627,"time_zone_offset":-480}
func (r *instance) DeleteSnapshot(id string) (ok bool, err error) {
	url := r.common.Command("/commands/delete_snapshot.action")
	params := make(map[string]string, 0)
